...
//                int weatherId;
                ...
                if(i == 0) {
                    icon = BitmapFactory.decodeResource(context.getResources(), Utility.getArtResourceForWeatherCondition(weatherId));
                    asset = createAssetFromBitmap(icon);
                    String convertedLow = Utility.formatTemperature(context, low);
                    String convertedHigh = Utility.formatTemperature(context, high);
                    min = convertedLow;
                    max = convertedHigh;
                    if(mGoogleApiClient != null) {
                        ...
                        putDataMapRequest.getDataMap().putString("min", convertedLow);
                        putDataMapRequest.getDataMap().putString("max", convertedHigh);
                        //Bitmap
                        // icon = BitmapFactory.decodeResource(context.getResources(), R.drawable.art_fog);

                        putDataMapRequest.getDataMap().putInt("asset", weatherId);
                                ...
                                .setResultCallback(new ResultCallback<DataApi.DataItemResult>() {
                                    ...
                                    public void onResult(@NonNull DataApi.DataItemResult dataItemResult) {
                                        if(!dataItemResult.getStatus().isSuccess()) {
                                            Log.v(LOG_TAG, "failure response");
                                        } else {
                                            Log.v(LOG_TAG, "success response");
                                        }
                                    }
                                });
                    }
